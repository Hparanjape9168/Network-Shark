...
class ServerThread extends Thread {
    ...
    public ServerThread(SocketServer _server, Socket _socket) {
        ...
        socket = _socket;
        ID = socket.getPort();
        ui = _server.ui;                    //pass the ref. of object of server_frame
    }

    public void send(Message msg) {
        try {
            streamOut.writeObject(msg);                        //serealize the msg
            streamOut.flush();
    ...
    public void open() throws IOException {
        ...
        streamOut.flush();
        streamIn = new ObjectInputStream(socket.getInputStream());          //deserealize msg of send by output object
    }
...
public class SocketServer implements Runnable {
    ...
    public ServerThread clients[];                            //array of thread for request
    public ServerSocket server = null;
    public Thread thread = null;
    public int clientCount = 0, port = 8844;
    public ServerPanel ui;
    public Database db;

    public SocketServer(ServerPanel frame) {

        clients = new ServerThread[50];                       //define array
        ui = frame;
        ...
        try {
            ...
            ui.jTextArea1.append("Server start. IP : " + InetAddress.getLocalHost() + ", Port : " + server.getLocalPort());
            start();                                        //start thread & waiting for clients
    ...
    public SocketServer(ServerPanel frame, int Port) {
    ...
    public void run() {
        while (thread != null) {
            try {
                ...
                addThread(server.accept());                 //accept the request of client
            } catch (Exception ioe) {
                ui.jTextArea1.append("\nServer accept error: \n");
                ui.RetryStart(0);
            }
